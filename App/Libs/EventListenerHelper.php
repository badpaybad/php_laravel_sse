...
class EventListenerHelper
{
    ...
    static function splitCompoundStringIntoParts($name, $value)
    {
    ...
    static function sanitiseVariableValue($name, $value)
    {
        $value = trim($value);
        if (!$value) {
            return array($name, $value);
    ...
    static function sanitiseVariableName($name, $value)
    {
        return array(Parser::parseName($name), $value);
    }
    static function resolveNestedVariables($value)
    {
        if (strpos($value, '$') !== false) {
            $value = preg_replace_callback(
                '/\${([a-zA-Z0-9_.]+)}/',
                function ($matchedPatterns) {
                    $nestedVariable = self::get_env($matchedPatterns[1]);
    ...
    static function normaliseEnvironmentVariable($name, $value)
    {
    ...
    {        
        ...
        foreach ($lines as $name) {
            if (!self::isComment($name) && self::looksLikeSetter($name)) {                
                ...
                list($name, $value) = self::normaliseEnvironmentVariable($name, null);
    ...
    public function SendNotifyToUser($tenantId, $userId, $msgJsonString)
    {
        ...
        $listSubscriber = $this->GetThenRemoveSubscriberExpired($userOnlineKey, $listSubscriber);
